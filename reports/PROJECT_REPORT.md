...
|----------|----------|---------|
| **`codeforces_master_dataset.csv`** | `Kaggle_dataset/` | **Baseline master — all features, 11,156 × 58.** The Kaggle upload. |
| `README.md`, `DATA_DICTIONARY.md` | `Kaggle_dataset/` | Dataset description + column reference for Kaggle. |
| `cleaned_dataset.csv` | root | Intermediate 51-column cleaned file (superset minus the last engineered additions). |
| `model_ready/` | root | 80:20 grouped train/test splits per task (rating & tags). |
| `models/` | root | `pipeline.py` (reproducible Phase 2–4), trained-model metrics, test predictions. |
| `PROJECT_REPORT.md` | `reports/` | **This consolidated report.** |

The raw input was `cf_problem_metadata.csv` (17 columns); all derived columns are deterministic functions of it.
...
### 10.2 Path B — Tags (multi-label, One-vs-Rest)
...
- **Reproducible:** every derived column is a deterministic function of the preserved text; splits and models use seed 42. `models/pipeline.py` regenerates Phase 2–4 end-to-end.
- **Offline substitutions:** Porter **stemming** replaces WordNet lemmatization, and **sentence-transformer embeddings were unavailable** (model downloads blocked in the build environment). Both are the top "if run locally" upgrades.
- **Highest-leverage next steps:** boost tag recall via per-label thresholds / `class_weight='balanced'`; add transformer embeddings; for rating, try ordinal binning or quantile loss and ensemble LightGBM with the linear model; handle the tag tail (merge/drop <15-example tags, model `*special`/`communication` separately).
