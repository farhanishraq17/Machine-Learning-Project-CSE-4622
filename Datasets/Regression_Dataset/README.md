...
## Suggested usage
Drop the ID columns from `X`, fit a `TfidfVectorizer` on `statement_proc` (train only), concatenate the numeric features, and evaluate with `GroupKFold` on `cv_group`. A reference pipeline (LightGBM champion, test MAE ≈ 404, R² ≈ 0.50) is in `../models/`.
