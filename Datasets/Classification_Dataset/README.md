...
## Suggested usage
Fit `TfidfVectorizer` on `statement_proc` (train only), `MultiLabelBinarizer` on `tags_norm`, train One-vs-Rest Logistic Regression or Linear SVM, evaluate with `GroupKFold` on `cv_group`. A reference pipeline (TF-IDF + Linear SVM champion, test micro-F1 ≈ 0.42) is in `../models/`.
